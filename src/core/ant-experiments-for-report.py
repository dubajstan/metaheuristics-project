...
def run_experiments_for_report(problem_name: str, max_fes: int, start_seed : int = 42, runs: int = 10) -> None:
    ...
    for i in range(runs):
        # Change the seed in each iteration to check algorithm stability
    ...
    std_dev_costs = float(np.std(best_costs))
    optimal_cost = float(optimal_cost)
    
   
    global_best_rpd = float((global_best - optimal_cost) / optimal_cost * 100.0)
    average_best_rpd = float((average_best - optimal_cost) / optimal_cost * 100.0)
    ...
    output_filename = RESULTS_DIR / f"{problem_name}_AS_experiment_statistics.json"
