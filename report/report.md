...
The results are competitive with the remaining algorithms tested in this report. On the largest `a280` instance, the Bees Algorithm obtained a slightly better best result and a better average RPD than Simulated Annealing, while using a smaller fixed evaluation budget. This improvement is mainly caused by the constructive nearest-neighbour initialization combined with intensive 2-opt neighbourhood search around selected sites. At the same time, the comparison should be interpreted in the context of different search dynamics: Simulated Annealing follows one solution trajectory, while the Bees Algorithm divides its budget between constructive scouts, several selected sites, elite neighbourhood searches, non-elite neighbourhood searches and random scouts.
...
**Simulated Annealing** - the maximum number of cost function evaluations was set to 5 000 000, with an additional stagnation limit of 50 000 iterations without improvement. In practice, the tested runs usually terminated earlier due to stagnation.

**Bees Algorithm** - the maximum number of cost function evaluations was set to 50 000. No additional stagnation criterion was used in the final experiment, because the goal was to observe the full behaviour of the population-based search under a fixed evaluation budget.
...
| `Bees Algorithm` | 10711.0 | 0.78% | 10857.0 | 2.15% | 1.74 | 1.15% |
...
| `Bees Algorithm` | 647.0 | 2.86% | 655.5 | 4.21% | 1.90 | 0.82% |
...
| `Bees Algorithm` | 2763.0 | 7.13% | 2815.7 | 9.18% | 2.77 | 0.89% |
...
The implemented Ant System, being the simplest variant of this algorithm, does not find optimal solutions. For the smaller tested instances, its results can be considered satisfactory. However, for the a280 instance, the best solution was noticeably worse - the best RPD was `14.93%`, while the average RPD reached `15.98%`.

The Ant System is stable - the best solutions it finds have a low standard deviation of the RPD. The execution time of the algorithm increases noticeably with the size of the optimized instance.
